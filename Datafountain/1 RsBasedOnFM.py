...
# @File : 1 RsBasedOnFM.py
# @Desc : 
# @Time : 2021/9/16 19:38 
# @Author : HanYun.
# @Version：V 1.0
# @Software: PyCharm
# @Related Links:
...
def vectorize_dic(dic, ix=None, p=None, n=0, g=0):
    ...
    for k, lis in dic.items():
        # users 和 users 的 list, 或者是 items 和 items 的 list
        for t in range(len(lis)):
            # 为编号为 t 的 user 或者 item 赋值
...
x_train = x_train.todense()  # todence() 返回矩阵
...
# tqdm 是一个快速，可扩展的Python的进度条。
from tqdm import tqdm
total = min(len(x_train), len(y_train))/batch_size
for epoch in range(epochs):
    loss_epoch = 0.0
    loss_all = 0.0
    perm = np.random.permutation(x_train.shape[0])
    model.train()
    for x, y in tqdm(batcher(x_train[perm], y_train[perm], batch_size), total=total):
        model.zero_grad()
